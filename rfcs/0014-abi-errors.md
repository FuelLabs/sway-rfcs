...
language tools and do not burden on chain artifacts with error strings.
...
defined types and error strings minimally burden the final bytecode size.
...
An error type implements the `AbiError` trait, but it is not recomended that you
do so manually.
...
The recommended way of doing this is to use the the `panic` intrinsic.
...
Either it is used with a `str` litteral (or a const-evaluable such value) and
...
that may conflict with either the reserved code range or the autogenerated panic
code range.
...
metaprogramming features such that developers may be able to define their own
language features of this caliber.

Given the benefits of having standardized errors, these tradeoffs seem
...
a meaningful anwser. Errors are part of making this answer meaningful and it is
