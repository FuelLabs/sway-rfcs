...
This RFC introduces the concept of a `StorageHandler` which describes a particular storage slot via its key. The RFC also reworks how storage accesses are represented in the language and how `StorageHandler` can be used to build dynamic storage types, such as `StorageMap` and `StorageVec`, more robustly.
...
1. Dynamic storage types, such as `StorageMap` and `StorageVec`, currently require a "hacky" compiler intrinsic called `__get_storage_key` which is not very well defined and is hard to use. Introducing the concept of a `StorageHandler` will helps us completely remove this intrinsic.
...
A "storage handler" is a thin wrapper type, in the standard library, around a `b256` key that describes a storage slot:
...
Reading and writing the data stored at `key` can be accomplished using the `get` and `store` methods below:
...
Notice that `get` returns an `Option` because it is possible that a particular storage slot is not valid, i.e. has not been written before, in which case `None` is returned. If the type `T` spans multiple storage slots, then the method `get` returns `None` if at least one the storage slots that `T` spans is invalid.

With the `StorageHandler` type available, this RFC proposes a redefinition of the meaning of the expression `storage.<var>.<field>...` to return a `StorageHandler` "pointing" to the actual data instead of returning the data itself. The RFC also proposes removing the "reassignment" statement `storage.<var>.<field>.. = ..` from the language.

Below is an example showing the behavior of `StorageHandler` and the new behavior of storage access expressions:
...
fn foo() {
    // Get a storage handler to the struct field `x` directly and then use it to store `42` in `x`.
    let x_handler: StorageHandler<u64> = storage.s.x;
    x_handler.store(42);
   
    // Call `store` and `get` directly on `storage.s.y`
    storage.s.y.store(ZERO_B256);
    let y:Option<b256> = storage.s.y.get();

    // Get a storage handler to the struct `s` and then use it to store `MyStruct { x: 42, y: ZERO_B256 }` in `s`.
    let s_handler: StorageHandler<MyStruct> = storage.s;
    s_handler.store(MyStruct { x: 42, y: ZERO_B256 } );

    // Read the struct `s`
    let s:Option<MyStruct> = s_handler.get();
}
```
...
impl TestContract for Contract {
    ...
    fn increment_counter(amount: u64) -> u64 {
        ...
        let incremented = storage.counter.get().unwrap_or(0) + amount;
...
The compiler will handle storage slots assignment for each storage variable. Each storage access expression will simply return a `StorageHandler` containing the key chosen by the compiler.
...
The typed expression `TyStorageAccess` in the compiler should now return `std::storage::StorageHandler` which should store the key chosen by the compiler according to the rules described in [Storage Slots Assignment](#storage-slot-assignment). The rest of the flow is be handled automatically after removing all the unnecessary logic in the compiler for reading and writing storage slots.

## Implementing Dynamic Storage Types using `StorageHandler`.

Dynamic storage types such `StorageMap` and `StorageVec` currently use the intrinsic `__get_storage_key` which is not very well defined and has a fragile implementation. With `StorageHandler` available and returned directly from a storage access expression (as in `storage.my_map`), we can re-implement methods like `StorageMap::insert`, `StorageMap::get`, and `StorageMap::remove` in a safer and cleaner way as follows:
...
Note that this requires that the compiler allows `self` to be type-ascribed. This is not possible today but should be easy to do. We should make sure to restrict the types allowed to only `Self` and `StorageHandler<Self>`. Rust has a small list of types that `self` is allowed to have such as `Rc` and `Arc` so this feature is not unheard of.
...
1. Reading and writing a storage variable is now a bit more complicated and less ergonomic than before because we now have to call `store` and `get` manually. We also have to handle the `Option` returned by `get` which can be quite verbose. We should consider making this more ergonomic by introducing a simpler syntax that de-sugars to the API calls above. For example, we could de-sugar something like `storage.x = 42` to `storage.x.store(42)`. We could also consider making `get` return the data directly instead of an `Option` if we completely disallow storage slots corresponding to storage variables from being invalidated (For example, remove `std::storage::clear`).

2. The second drawback is related to the fact that structs take up more storage slots than they actually need. This behavior is **not** new. There is a tradeoff here to be considered. If we make structs tightly packed, then manipulating a single field becomes more expensive (potential read-modify-write patterns or fields "spilling" over to the next slot), but the number of storage slots required may become lower. We might want to consider a `#[packed]` attribute in the future to let the user decide on one way v.s. the other. Another thing to consider with tightly-packed structs is that the type `StorageHandler` would then also require an additional field that points to a particular location _inside_ a given slot (i.e. which byte to start reading from or writing to).

# Rationale and alternatives

[rationale-and-alternatives]: #rationale-and-alternatives

One alternative is to continue with the current use model which has many flaws so that's not ideal. Another alternative is to introduce an `AsStorageHandler` trait that allows calling `storage.x.as_storage_handler()` to get the storage handler manually, but that approach has its limitations when implementing `StorageMap` and `StorageVec` and requires defining the behavior of `as_storage_hander` when called on a regular stack variable.

One more alternative is to completely remove the concept of a "storage variable" and switch over to thinking about storage like file I/O or databases. However, I think the approach described in this RFC provides a good middle ground where methods like `store` and `get` somewhat behave like I/O methods but also keeps the abstraction of "storage variables" to help user write readable code.
...
In Rust, the closest thing to storage accesses is file I/O where a handler is used to read and write to a file similarly to `store` and `get` above.
...
- How do we obtain a `StorageHandle` for data stored in a dynamic storage collection such as `StorageMap` or `StorageVec`? Is that even needed/required?
