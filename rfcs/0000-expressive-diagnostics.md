...
- _Help_: Additional friendly information, which is not related to a place in code, that helps understanding and solving the issue.
...
`issue` and `info` elements of `#[error]` take `Span` as their first argument, as seen above. If the passed `Span` parameter equals `Span::dummy()` the _label_ is considered _not to be in source code_.
